...
// .wrangler/tmp/pages-7sHxJ8/functionsWorker-0.28141308777643914.mjs
...
__name2(tokenClaims, "tokenClaims");
async function verifyAdmin(request, env) {
  const authorization = request.headers.get("Authorization");
  if (!authorization?.startsWith("Bearer ")) {
    throw new HttpError(401, "La sesi\xF3n no es v\xE1lida. Volv\xE9 a ingresar.");
  }
  ...
  if (!response.ok) {
    throw new HttpError(401, "La sesi\xF3n venci\xF3. Volv\xE9 a ingresar.");
  }
  ...
  if (claims.sub !== account.localId || claims.admin !== true) {
    throw new HttpError(403, "Esta cuenta no tiene permiso para realizar esta acci\xF3n.");
  }
  return {
    token,
    uid: account.localId,
...
function errorResponse(error) {
  ...
  console.error(error);
  return jsonResponse({ message: "No se pudo completar la operaci\xF3n." }, 500);
}
...
__name(toHex, "toHex");
__name2(toHex, "toHex");
async function cloudinarySignature(parameters, secret) {
  const serialized = Object.entries(parameters).filter(([, value]) => value !== "").sort(([left], [right]) => left.localeCompare(right)).map(([key, value]) => `${key}=${value}`).join("&");
...
__name2(cloudinarySignature, "cloudinarySignature");
async function onRequestPost({ request, env }) {
  try {
    await verifyAdmin(request, env);
    const body = await request.json().catch(() => null);
    if (typeof body?.publicId !== "string" || body.publicId.length > 220) {
      throw new HttpError(400, "La imagen indicada no es v\xE1lida.");
    }
...
__name(metaUrl, "metaUrl");
__name2(metaUrl, "metaUrl");
async function currentContentVersion(projectId, token) {
  const response = await fetch(metaUrl(projectId), {
    headers: { Authorization: `Bearer ${token}` }
  });
  if (response.status === 404) return 0;
  if (!response.ok) throw new HttpError(502, "No se pudo leer el estado del cat\xE1logo.");
  const data = await response.json();
  return Number(data.fields?.contentVersion?.integerValue ?? 0);
}
__name(currentContentVersion, "currentContentVersion");
__name2(currentContentVersion, "currentContentVersion");
async function markPublicationRequested(projectId, token, version) {
  const masks = [
  ...
  const response = await fetch(`${metaUrl(projectId)}?${masks}`, {
    ...
    body: JSON.stringify({
      fields: {
        ...
        lastPublishRequestedAt: { timestampValue: (/* @__PURE__ */ new Date()).toISOString() },
        publishStatus: { stringValue: "queued" }
      }
    })
  });
  if (!response.ok) throw new HttpError(502, "El build comenz\xF3, pero no se pudo actualizar su estado.");
}
__name(markPublicationRequested, "markPublicationRequested");
__name2(markPublicationRequested, "markPublicationRequested");
async function onRequestPost3({ request, env }) {
  try {
    const admin = await verifyAdmin(request, env);
    if (!env.FIREBASE_PROJECT_ID) throw new HttpError(500, "Falta configurar Firebase en Cloudflare.");
    if (!env.CLOUDFLARE_DEPLOY_HOOK_URL) throw new HttpError(500, "Falta configurar el enlace de publicaci\xF3n.");
    ...
    if (!hookResponse.ok) throw new HttpError(502, "Cloudflare no pudo iniciar la publicaci\xF3n.");
...
// .wrangler/tmp/bundle-LWGRh3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LWGRh3/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.28141308777643914.js.map
